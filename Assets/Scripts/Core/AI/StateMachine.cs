...
public class StateMachine<T> : IStateMachine where T : struct, IConvertible, IComparable
{
	...
	StateMachineRunner engine;
	MonoBehaviour component;

	StateMapping lastState;
	StateMapping currentState;
	StateMapping destinationState;

	Dictionary<object, StateMapping> stateLookup;
	...
	V CreateDelegate<V>(MethodInfo method, object target) where V : class
	{
		...
		if (ret == null) throw new ArgumentException("Unabled to create delegate for method called " + method.Name);
		return ret;
	...
	{
		if (stateLookup == null) throw new Exception("States have not been configured, please call initialized before trying to set state");

		if (!stateLookup.ContainsKey(newState)) throw new Exception("No state with the name " + newState.ToString() + " can be found. Please make sure you are called the correct type the statemachine was initialized with");
		...
		switch (transition)
		{
			...
			case StateTransition.Overwrite:
				if (currentTransition != null) engine.StopCoroutine(currentTransition);
				if (exitRoutine != null) engine.StopCoroutine(exitRoutine);
				if (enterRoutine != null) engine.StopCoroutine(enterRoutine);
		...
		else //Same frame transition, no coroutines are present
		{
			...
			if (currentState != null)
			{
				...
				if (Changed != null) Changed((T) currentState.state);
			}
	...
	IEnumerator ChangeToNewStateRoutine(StateMapping newState, StateTransition transition)
	{
		...
		if (currentState != null)
		{
			if (currentState.hasEnterRoutine)
			{
				...
				if (enterRoutine != null) yield return engine.StartCoroutine(enterRoutine);
	...
	public T State { get { return (T) currentState.state; } }

	public bool IsInTransition { get { return isInTransition; } }

	public StateMapping CurrentStateMap { get { return currentState; } }

	public MonoBehaviour Component { get { return component; } }
	...
	{
		StateMachineRunner engine = component.GetComponent<StateMachineRunner>();
