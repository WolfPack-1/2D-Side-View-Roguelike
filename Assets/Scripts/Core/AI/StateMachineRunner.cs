...
public class StateMachineRunner : MonoBehaviour
{
	...
	{
		StateMachine<T> fsm = new StateMachine<T>(this, component);
	...
	{
		StateMachine<T> fsm = Initialize<T>(component);
	...
	void FixedUpdate()
	{
		...
		{
			IStateMachine fsm = stateMachineList[i];
	...
	void Update()
	{
		...
		{
			IStateMachine fsm = stateMachineList[i];
			if (!fsm.IsInTransition && fsm.Component.enabled) fsm.CurrentStateMap.Update();
		}
	}
	...
	void LateUpdate()
	{
		...
		{
			IStateMachine fsm = stateMachineList[i];
			if (!fsm.IsInTransition && fsm.Component.enabled) fsm.CurrentStateMap.LateUpdate();
		}
	}
